...
const ProductDetails: React.FC<ProductDetailsProps> = ({ product }) => {
  ...
  if (specifications && specifications.length > 0) {
    specifications.forEach((spec: { name: string; value: string }) => {
  ...
  const otherMarketplaces =
    marketplaceLinks?.others?.map(
      (marketplace: { name: string; url: string; logo?: string }) => ({
  ...
  const formattedDistributors =
    distributors?.map(
      (distributor: { name: string; url: string; location: string }) => ({
