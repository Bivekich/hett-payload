...
function hasContent(node: LexicalNode): boolean {
  ...
  if (
    ...
    return (node as LexicalParagraphNode).children.some((child) =>
      hasContent(child as LexicalNode)
    );
  }
  ...
  if (node.type === 'link') {
    ...
    if (linkNode.children && linkNode.children.length > 0) {
      return linkNode.children.some((child) =>
        hasContent(child as LexicalNode)
      );
    }
  }
  ...
  const htmlParts = filteredNodes.map((node: LexicalNode) => {
    ...
    if (node.type === 'quote') {
      ...
      const paragraphs = quoteNode.children
        ...
        .map((child) => {
          if (child.type === 'paragraph') {
            const content = processTextChildren(
              (child as LexicalParagraphNode).children
            );
      ...
      return paragraphs.length
        ? `<blockquote>${paragraphs.join('')}</blockquote>`
        : '';
    }
...
function processLinkNode(node: LexicalLinkNode): string {
  ...
  return content.trim()
    ? `<a href="${node.url}" target="_blank" rel="noopener noreferrer">${content}</a>`
    : '';
}
